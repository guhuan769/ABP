...
namespace GMap.NET.CacheProviders
{
   ...
   public class SQLitePureImageCache : PureImageCache
   {
      ...
      public static System.Reflection.Assembly CurrentDomain_AssemblyResolve(object sender, ResolveEventArgs args)
      {
         ...
         {
            string rootDir = Application.StartupPath + Path.DirectorySeparatorChar + "GMap.NET" + Path.DirectorySeparatorChar;
      ...
      public static bool DeleteData()
      {
          ...
          try
          {
              string file = Application.StartupPath + System.IO.Path.DirectorySeparatorChar + "GMap.NET" + System.IO.Path.DirectorySeparatorChar + "TileDBv5" + Path.DirectorySeparatorChar + GMapProvider.LanguageStr + Path.DirectorySeparatorChar + "Data.gmdb";
                  ...
                  using (SQLiteConnection cn = new SQLiteConnection())
                  {
                      ...
                      cn.Open();
                      {
                          using (DbCommand com = cn.CreateCommand())
                          {
                              ...
                              com.CommandText = string.Format("DELETE FROM Tiles;DELETE FROM TilesData;");
         ...
         if(Created)
         {
            try
            {
                if (Common.IsDownLoad)
                {
                    ...
                    image.Save(string.Format("{0}\\{1}\\{2}_{3}.png",Common.ImageSavePath,zoom,pos.X,pos.Y), System.Drawing.Imaging.ImageFormat.Png);
                }
                else
                {
                    using (SQLiteConnection cn = new SQLiteConnection())
