      ...
      public PureImage GetImageFrom(GMapProvider provider, GPoint pos, int zoom, out Exception result)
      {
         ...
         try
         {
             if (HideMap)
             {
                 ...
                 Properties.Resources.image_none.Save(ms, System.Drawing.Imaging.ImageFormat.Png);
                 ret = GMapProvider.TileImageProxy.FromStream(ms);
                     ...
                     if (Mode != AccessMode.CacheOnly)
                     {
                         ret = provider.GetTileImage(pos, zoom);
                         {
                             ...
                             if (ret != null)
                             {
                                 ...
                                 {
                                     System.Drawing.Image image = System.Drawing.Image.FromStream(ret.Data);
                                     string path = string.Format("{0}\\{1}", Common.ImageSavePath, zoom);
                                     if (!Directory.Exists(path))
                                     {
                                         Directory.CreateDirectory(path);
                                     }
                                     image.Save(string.Format("{0}\\{1}_{2}.png", path, pos.X, pos.Y), System.Drawing.Imaging.ImageFormat.Png);
                                 }
                             }
                         }
                     }
