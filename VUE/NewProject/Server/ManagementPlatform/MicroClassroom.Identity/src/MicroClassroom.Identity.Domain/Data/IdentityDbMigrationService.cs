...
public class IdentityDbMigrationService : ITransientDependency
{
    ...
    private async Task AddInitTenantListAsync()
    {
        var section = _configuration.GetSection("Tenants");
        if (section == null)
        {
            return;
        }

        var importDtos = new List<ImportTenantDto>();
        section.Bind(importDtos);

        var tenantList = new List<Tenant>();
        foreach (var dto in importDtos)
        {
            var tenant = await _tenantManager.CreateAsync(dto.Name);
            if (!dto.ConnectionString.IsNullOrWhiteSpace())
            {
                tenant.SetConnectionString("Default", dto.ConnectionString);
            }
            tenantList.Add(tenant);
        }

        await _tenantRepository.InsertManyAsync(tenantList);
    }
